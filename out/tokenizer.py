...
class Encoder(go.GoClass):
	...
	def BPE(self, token):
		"""BPE(str token) []str"""
		return go.Slice_string(handle=_tokenizer.tokenizer_Encoder_BPE(self.handle, token))
	def EncodeWords(self, words):
		"""EncodeWords([]str words) []long"""
		return go.Slice_int64(handle=_tokenizer.tokenizer_Encoder_EncodeWords(self.handle, words.handle))
...
def WordSplit(s):
	"""WordSplit(str s) []str"""
	return go.Slice_string(handle=_tokenizer.tokenizer_WordSplit(s))
